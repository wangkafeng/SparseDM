...
def train(config):
    ...
    train_state = utils.load_train_state_c10(config, device)    #    from  pretrained  
    ...
    # train_state.resume(config.ckpt_root)   #    close for mutli step
    ...
    step_fid = []
    g_steps = 0   #    add
    board_steps = [100000, 200000, 300000, 400000]
    # board_steps = [10, 20, 30, 40]
    while train_state.step < config.train.n_steps:
        #    add for multi steps
        if g_steps==board_steps[0]:  # 6:8
            train_state = utils.load_train_state_c10_multi_step6_8(config, device, step=g_steps)    #    from  pretrained  
            nnet, nnet_ema, _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
        if g_steps==board_steps[1]:   # 5:8
            train_state = utils.load_train_state_c10_multi_step5_8(config, device, step=g_steps)    #    from  pretrained  
            nnet, nnet_ema, _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
            # print('after load_train_state_c10_multi_step7  nnet_ema = ', nnet_ema)
        elif  g_steps ==board_steps[2]:     #  4:8
            train_state = utils.load_train_state_c10_multi_step4_8(config, device, step=g_steps)    #    from  pretrained  
            nnet, nnet_ema,  _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
            # print('after load_train_state_c10_multi_step3  nnet_ema = ', nnet_ema)
        elif g_steps ==board_steps[3]:     #  2:4
            train_state = utils.load_train_state_c10_multi_step2(config, device, step=g_steps)    #    from  pretrained  
            nnet, nnet_ema,  _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
            # print('after load_train_state_c10_multi_step2  nnet_ema = ', nnet_ema)
        ...
        g_steps += 1   #    add for multi steps,  replace   train_state.step+=1
        train_state.step = g_steps  #    add for multi steps 
        ...
        if train_state.step % config.train.save_interval == 0 or train_state.step == config.train.n_steps:
            ...
            accelerator.wait_for_everyone()
            #    
            fid = eval_step(n_samples=10000, sample_steps=50, algorithm='dpm_solver')  # calculate fid of the saved checkpoint   #    modify
            # fid = 0  #    add for test
