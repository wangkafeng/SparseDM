...
def train(config):
    ...
    # train_state = utils.load_train_state_c10_pd(config, device)    #    from  pretrained
    train_state = utils.load_train_state_c10_ls(config, device)  
    # train_state, teacher_nnet_ckpt = utils.load_train_state_c10_ls(config, device)  
    ...
    # teacher_nnet, teacher_nnet_ema, nnet, nnet_ema, optimizer, train_dataset_loader = accelerator.prepare(
    #     train_state.teacher_nnet, train_state.teacher_nnet_ema, train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)  
    # teacher_nnet_ckpt = torch.load('./pretrained_models/cifar10_uvit_small.pth', map_location='cpu')
    # teacher_nnet = utils.get_nnet(**config.teacher_nnet).to(device)
    # print('....................')
    # print('teacher_nnet = ', teacher_nnet)
    # teacher_nnet.eval()
    ...
    #     teacher_nnet_ckpt, train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)  
    teacher_nnet_ema, nnet, nnet_ema, optimizer, train_dataset_loader = accelerator.prepare(
    ...
    def train_step(_batch):
        ...
        if config.train.mode == 'uncond':  #    run this
            # loss = sde.LSimple(score_model, _batch, pred=config.pred)   #    modify
            # loss = sde.LSimple(score_model, _batch, teacher_nnet, pred=config.pred) 
            loss = sde.LSimple(score_model, _batch, teacher_nnet_ema, pred=config.pred) 
            # loss = sde.LSimple(score_model, _batch, teacher_nnet_ckpt, pred=config.pred) 
            ...
            target = teacher_nnet(xt, t * 999)   #    modify for distillation_loss_simple ,  loss down fast , fid why ?
    ...
    def eval_step(n_samples, sample_steps, algorithm):
        ...
        def sample_fn(_n_samples):
            ...
            if algorithm == 'euler_maruyama_sde':
                return sde.euler_maruyama(sde.ReverseSDE(score_model_ema), _x_init, sample_steps, **kwargs)  #    run this,  _ema hasn't update ?
