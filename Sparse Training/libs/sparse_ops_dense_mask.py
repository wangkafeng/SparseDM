...
class SparseLinear(nn.Linear):
    ...
    def get_sparse_weights_inference(self, N, M):   #      only for inference
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # print('SparseLinear forward')
            
            w, mask = self.get_sparse_weights()  #      fix sparse ratio

            # w, mask = self.get_sparse_weights_inference(N=2, M=4)   #      dynamic mask from weights,   only for inference
            # w, mask = self.get_sparse_weights_inference(N=2, M=8)   #      pass
            # w, mask = self.get_sparse_weights_inference(N=7, M=8)
            # w, mask = self.get_sparse_weights_inference(N=1, M=8)
            # w, mask = self.get_sparse_weights_inference(N=1, M=16)
            # w, mask = self.get_sparse_weights_inference(N=15, M=16)
            # w, mask = self.get_sparse_weights_inference(N=31, M=32)
            # w, mask = self.get_sparse_weights_inference(N=1, M=32)
            
            #      add for mask > 0
            # w, _ = self.get_sparse_weights_inference(N=2, M=4) 
            # mask = (w > 0)
