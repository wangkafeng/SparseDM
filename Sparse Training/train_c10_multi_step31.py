...
def train(config):
    ...
    while train_state.step < config.train.n_steps:
        #     add for multi steps
        if g_steps==board_steps[0]:  # 15:16
            train_state = utils.load_train_state_c10_multi_step15(config, device, step=g_steps)    #     from  pretrained  
            nnet, nnet_ema, _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
        if g_steps==board_steps[1]:   # 7:8
            train_state = utils.load_train_state_c10_multi_step7(config, device, step=g_steps)    #     from  pretrained  
            nnet, nnet_ema, _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
            # print('after load_train_state_c10_multi_step7  nnet_ema = ', nnet_ema)
        elif  g_steps ==board_steps[2]:     #  3:4
            train_state = utils.load_train_state_c10_multi_step3(config, device, step=g_steps)    #     from  pretrained  
            nnet, nnet_ema,  _, _ = accelerator.prepare(
                train_state.nnet, train_state.nnet_ema, train_state.optimizer, train_dataset_loader)
