...
class SDE(object):
    ...
    def cum_beta(self, t):  # the variance of xt|x0
        raise NotImplementedError     #    run VPSDE 
    ...
    def sample(self, x0, t_init=0):  # sample from q(xn|x0), where n is uniform
        t = torch.rand(x0.shape[0], device=x0.device) * (1. - t_init) + t_init   #    this must use teachear sample ? not random sample ?
...
class ScoreModel(object):
    r"""
        The forward process is q(x_[0,T])     #    this like ldm ["eps", "x0"] ?
    """

    def __init__(self, nnet: nn.Module, pred: str, sde: SDE, T=1):
        assert T == 1
        self.nnet = nnet   #    teacher_nnet
        self.pred = pred
        self.sde = sde
        self.T = T
...
class ScoreModelPD(object):
    r"""
        The forward process is q(x_[0,T])     #    this like ldm ["eps", "x0"] ?
    """

    # def __init__(self, nnet: nn.Module, pred: str, sde: SDE, T=1):
    def __init__(self, nnet: nn.Module, teacher_nnet: nn.Module, pred: str, sde: SDE, T=1):
        assert T == 1
        self.nnet = nnet   #    student_nnet
        self.teacher_nnet = teacher_nnet  #    add teacher_nnet
        self.pred = pred
        self.sde = sde
        self.T = T
        print(f'ScoreModel with pred={pred}, sde={sde}, T={T}')   #  ScoreModel with pred=pd, sde=vpsde beta_0=0.1 beta_1=20, T=1
...
# def LSimple(score_model: ScoreModel, x0, pred='noise_pred', **kwargs):  #    modify
def LSimple(score_model: ScoreModel, x0, teacher_model, pred='noise_pred', **kwargs):
    t, noise, xt = score_model.sde.sample(x0)
    if pred == 'noise_pred':
        noise_pred = score_model.noise_pred(xt, t, **kwargs)   #    
        # return mos(noise - noise_pred)  #    modify

        # target = teacher_model(xt, t * 999)   #      mode='uncond'
        target = teacher_model(xt, t * 999, **kwargs)   # mode='cond'
        return mos(target - noise_pred) 
    elif pred == 'x0_pred':
        x0_pred = score_model.x0_pred(xt, t, **kwargs)
        return mos(x0 - x0_pred)
    if pred == 'pd':   #    modify for pd
        noise_pred = score_model.noise_pred(xt, t, **kwargs)   #    
        return mos(noise - noise_pred)
    else:
        raise NotImplementedError(pred)
