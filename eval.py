...
def evaluate(config):
    ...
    # use 2:4 sparsity acceleration
    nnet.cuda()
    if config.type == "sparse":
        nnet.module.sparse24()
    else:
        nnet.module.dense24()
    ...
    with tempfile.TemporaryDirectory() as temp_path:
        ...
        total_time = utils.sample2dir(accelerator, path, config.sample.n_samples, config.sample.mini_batch_size, sample_fn, dataset.unpreprocess)
        if accelerator.is_main_process:
            # fid = calculate_fid_given_paths((dataset.fid_stat, path))
            logging.info(f'nnet_path={config.nnet_path}, time={total_time}')
